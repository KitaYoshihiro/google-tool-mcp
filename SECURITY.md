...
Gmail ではメッセージ本文に加えて、明示的に指定した対応テキスト添付ファイルの内容を読み取れます。PDF、Office 文書、画像、zip などのバイナリ添付ファイルはテキストとして読み取りません。

Gmail 添付ファイルは、明示的に指定した場合にローカルファイルとしてダウンロードできます。保存先を指定しない場合は、profile 別の `~/.cache/google-tool/attachments/` 配下に保存します。MCP レスポンスには添付ファイル本文を含めず、保存先と metadata だけを返します。
